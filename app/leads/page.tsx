  ...
  useEffect(() => {
    fetch('/api/leads?limit=50')
      ...
      .then(data => {
        ...
        setLeads(fetched.length > 0 ? fetched : DEMO_LEADS)
        setTotal(data.total ?? (fetched.length > 0 ? fetched.length : DEMO_LEADS.length))
      })
      .catch(() => {
        setLeads(DEMO_LEADS)
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto">
        {loading ? (
          ...
          <div className="flex items-center justify-center h-32">
            <span className="text-[13px]" style={{ color: 'var(--color-muted)' }}>Aucun lead pour l'instant</span>
          </div>
        ) : (
