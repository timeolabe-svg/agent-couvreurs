    ...
    <div style="background:#1a1d27;border-radius:0 0 12px 12px;padding:24px 28px">
      ...
      <div style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px">
        ...
        <div style="display:flex;gap:16px;flex-wrap:wrap">
          ${contact.phone ? `<span style="font-size:13px;color:#e1e4e8">📞 <a href="tel:${contact.phone}" style="color:#22c55e;text-decoration:none">${contact.phone}</a></span>` : ''}
          <span style="font-size:13px;color:#e1e4e8">✉️ <a href="mailto:${contact.email}" style="color:#22c55e;text-decoration:none">${contact.email}</a></span>
        </div>
      </div>
      ...
      <div style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px">
        ...
        ${calendarEventUrl ? `<a href="${calendarEventUrl}" style="display:inline-block;margin-right:12px;font-size:13px;color:#22c55e;text-decoration:none">📆 Voir dans Google Calendar →</a>` : ''}
      ...
      <div style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px">
        ...
        <blockquote style="margin:0;padding:10px 14px;border-left:3px solid #22c55e;background:#0d1117;border-radius:0 4px 4px 0;font-size:13px;color:#c9d1d9;font-style:italic;line-height:1.5">${replyContext.replace(/\n/g, '<br>').substring(0, 500)}${replyContext.length > 500 ? '…' : ''}</blockquote>
      </div>` : ''}
      ...
      <!-- CTA -->
      <a href="${baseUrl}" style="display:inline-block;background:#22c55e;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-size:14px;font-weight:600;letter-spacing:.01em">Voir dans le dashboard →</a>
    </div>
