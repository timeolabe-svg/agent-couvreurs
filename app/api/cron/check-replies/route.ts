...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    type StaleRow = {
      draftId: string; sentAt: Date | null; replyId: string
      instantlyReplyId: string | null; contactId: string | null
    ...
    for (const stale of staleDrafts.rows) {
      try {
        ...
        const contact = contactById.get(stale.contactId as string)
        if (!contact) continue
    ...
    for (const reply of replies) {
      try {
        ...
        if (resolvedContact) {
          ...
          if (newEmail) {
            try {
              // Récupère la campagne du contact (pour ré-attacher la ligne propre) AVANT d'annuler.
              const [existingQ] = await db.select({ cid: email_queue.campaign_id })
                .from(email_queue).where(eq(email_queue.contact_id, resolvedContact.id)).limit(1)
              ...
              // ANTI-BOUCLE : on n'ÉCRASE JAMAIS les lignes déjà 'sent' (sinon le plafond à vie
              // repart de 0 et toute la séquence est renvoyée). On annule seulement la file
              // restante puis on insère UNE ligne step 0 propre pour la nouvelle adresse.
              await db.execute(sql`UPDATE email_queue SET status = 'cancelled' WHERE contact_id = ${resolvedContact.id} AND status IN ('pending', 'queued', 'queued_instantly')`)
              if (existingQ?.cid) {
                await db.insert(email_queue).values({
                  contact_id: resolvedContact.id,
                  campaign_id: existingQ.cid,
                  sequence_step: 0,
                  from_email: 'pending@hdigiweb.fr',
