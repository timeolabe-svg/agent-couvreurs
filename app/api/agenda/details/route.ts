...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const msgs = [
      ...
      ...received.filter(r => r.body).map(r => { const b = cleanIncomingBody(r.body); return { role: 'prospect' as const, body: stripQuotedReply(b) || b, ts: r.at ? new Date(r.at).getTime() : 0 } }),
