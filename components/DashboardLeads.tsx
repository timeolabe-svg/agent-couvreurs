...
import { AlertCircle, Clock, CheckCircle2, TrendingUp, Mail, MessageSquare, Calendar } from 'lucide-react'
...
export function DashboardLeads({ leads, onLeadsRefresh }: { leads: Lead[]; onLeadsRefresh?: () => void }) {
  const [showImport, setShowImport] = useState(false)
  const total = leads.length
  const active = leads.filter(l => !['not_interested', 'rdv_booked'].includes(l.stage)).length
  const replied = leads.filter(l => l.stage === 'replied').length
  const rdvs = leads.filter(l => l.stage === 'rdv_booked').length
  const replyRate = total > 0
    ? Math.round((leads.filter(l => l.stage !== 'prospected' && l.stage !== 'contacted' && l.stage !== 'not_interested').length / total) * 100)
    : 0

  const byRecent = (a: Lead, b: Lead) => new Date(b.lastActivityAt).getTime() - new Date(a.lastActivityAt).getTime()
  const needsAction = leads.filter(l => l.stage === 'replied').sort(byRecent)
  const inSequence  = leads.filter(l => ['contacted', 'follow_up_1', 'follow_up_2'].includes(l.stage)).sort(byRecent)
  const closed      = leads.filter(l => ['rdv_booked', 'not_interested'].includes(l.stage)).sort(byRecent)
